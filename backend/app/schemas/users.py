...
Enums are imported from `app.domain.enums`, never redeclared. The domain owns
the vocabulary; persistence, transport and policy must all speak the same one
(docs/03_REPOSITORY_STRUCTURE.md: "no agent may create duplicate domain
abstractions").
"""
...
class EligibilityRead(BaseModel):
    ...
    — the router computes no part of it. `reasons` carries stable
