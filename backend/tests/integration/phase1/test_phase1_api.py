...
class TestUsersAPI:
    ...
    def test_create_user_endpoint(self, client: TestClient) -> None:
        payload = {
    ...
    def test_get_user_profile(self, client: TestClient) -> None:
        # Create user first
        payload = {
        ...
        if create_resp.status_code not in (200, 201):
            assert False, f"User creation failed: {create_resp.text}"
    ...
    def test_get_nonexistent_user_returns_404_with_error_envelope(self, client: TestClient) -> None:
    ...
    def test_user_eligibility_endpoint(self, client: TestClient) -> None:
        payload = {
        ...
        if create_resp.status_code in (200, 201):
            user_id = create_resp.json()["id"]
            elig_resp = client.get(f"/api/v1/users/{user_id}/eligibility")
            assert elig_resp.status_code == 200
            data = elig_resp.json()
            assert "eligible" in data or "verification_level" in data or "status" in data


class TestSitesAndAssetsAPI:
    """Verify Sites, Meters, and Assets endpoints."""

    def test_create_and_get_site(self, client: TestClient) -> None:
        # Create user first
        ...
        if user_resp.status_code not in (200, 201):
            assert False, f"User creation failed: {user_resp.text}"
    ...
    def test_attach_meter_to_site(self, client: TestClient) -> None:
        # Setup user and site
        user_id = client.post(
            "/api/v1/users",
            json={
        ...
        if not user_id:
            assert False, "Could not create user"
        ...
        if not site_id:
            assert False, "Could not create site"

        meter_resp = client.post(
    ...
    def test_register_energy_asset(self, client: TestClient) -> None:
        # Setup user and site
        user_id = client.post(
            "/api/v1/users",
            json={
...
class TestGridNodesAPI:
    """Verify Grid Nodes endpoints."""

    def test_list_grid_nodes(self, client: TestClient) -> None:
        response = client.get("/api/v1/grid/nodes")
        assert response.status_code == 200, f"Failed to list grid nodes: {response.text}"
